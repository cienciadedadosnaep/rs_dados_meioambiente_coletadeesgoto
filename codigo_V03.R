...
dados <- subset(dados, classe == "q4_vol_esgoto_trat")
...
titulo<-T_ST_P_No_MEIOAMBIENTE$TITULO[4]
subtexto<-"Fonte: Painel do Saneamento"
link <- T_ST_P_No_MEIOAMBIENTE$LINK[4]
...
write(exportJson0,file = paste('data/',gsub('.csv','',T_ST_P_No_MEIOAMBIENTE$NOME_ARQUIVO_JS[4]),
                               '.json',sep =''))
write(texto,file = paste('data/',T_ST_P_No_MEIOAMBIENTE$NOME_ARQUIVO_JS[4],
                         sep =''))
